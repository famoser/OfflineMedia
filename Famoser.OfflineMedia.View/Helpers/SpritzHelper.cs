...
namespace Famoser.OfflineMedia.View.Helpers
{
    public class SpritzHelper
    {
        ...
        private static List<SpritzWord> ToSpritzWords(List<TextModel> models)
        {
            var list = new List<SpritzWord>();
            foreach (var textModel in models)
            {
                var li = ToSpritzWords(textModel);
                list.AddRange(li);
            }
            return list;
        }
